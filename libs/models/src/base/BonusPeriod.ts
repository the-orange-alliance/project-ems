...
import { Crescendo } from '../seasons/index.js';
...
export interface BonusPeriodSettings {
  duration: number;
  autoEnd: (
    matchAtStartState: Match<any>,
    currentMatchState: Match<any>
  ) => boolean;
}
...
export const FRC_2024_AMPLIFY_RED: BonusPeriodSettings = {
  duration: 10,
  autoEnd: (
    matchAtStartState: Match<Crescendo.MatchDetails>,
    currentMatchState: Match<Crescendo.MatchDetails>
  ) => {
    if (!matchAtStartState.details || !currentMatchState.details) return false;

    const totalSpeakerNotesAtStart =
      matchAtStartState.details.redTeleSpeakerNotesAmped;
    const totalSpeakersNotesNow =
      currentMatchState.details.redTeleSpeakerNotesAmped;
...
export const FRC_2024_AMPLIFY_BLUE: BonusPeriodSettings = {
  ...
  autoEnd: (
    ...
    const totalSpeakerNotesAtStart =
      matchAtStartState.details.blueTeleSpeakerNotesAmped;
    const totalSpeakersNotesNow =
      currentMatchState.details.blueTeleSpeakerNotesAmped;
